...
## Case1. Docker - Redis 컨테이너 실행(CMD, Power Shell)
...
## Case2. Windows - Redis 컨테이너 실행(CMD, Power Shell)
...
## Case3. p3x-redis-ui 함께 사용하기

```bash
# settings 폴더 생성
mkdir C:\docker\redis-ui
cd C:\docker\redis-ui
mkdir settings
...
# redis 이미지 생성
docker run -d --name redis-local --network redis-net -p 6380:6379 redis

# 실행
docker run -d --name p3x-redis-ui --network redis-net -p 127.0.0.1:7843:7843 -v ${PWD}\settings:/settings patrikx3/p3x-redis-ui:latest

# 오류나면 로그 확인
docker logs p3x-redis-ui --tail 200
```
